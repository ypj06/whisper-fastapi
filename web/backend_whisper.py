                ...
                if req.target_lang:
                    ...
                    if os.path.exists(srt_path) and not req.overwrite:
                        ...
                        with open(srt_path, "w", encoding="utf-8") as f:
                            f.write(srt_content)
                        log.append(f"  [{lang} -> {req.target_lang}] Translated SRT saved: {os.path.basename(srt_path)}")
                else:

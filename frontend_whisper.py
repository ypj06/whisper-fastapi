...
with st.sidebar:
    ...
    languages = st.multiselect("识别语言（源语言）", ["en", "zh", "ja", "ko", "fr", "de"], default=["en"])
...
with tab2:
    ...
    if uploaded_file and st.button("开始转写", type="primary"):
        with st.spinner("转写+翻译中..."):
            ...
            if resp.ok:
                data = resp.json()
                st.download_button("下载 SRT 字幕", data.get("srt", ""), file_name=f"{uploaded_file.name}.srt")
                st.download_button("下载 TXT 文本", data.get("txt", ""), file_name=f"{uploaded_file.name}.txt")
